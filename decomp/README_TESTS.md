...
```sh
python3 fetch_task_workbooks.py --export tasksexport20260904.json   # once
python3 run_tests.py            # diff + audit all five
python3 run_tests.py --rebuild  # re-diff the workbooks first (slow: ~20 min)
python3 test_teeth.py           # negative controls - prove the test can fail
```
