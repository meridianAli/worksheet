...
RULES = [
    RuleInfo("R001", "No cell / row / column references in the rubric", ERROR, "deterministic", ("rubric",),
    ...
    RuleInfo("R002", "Every criterion is a Yes/No question ending in '?'", ERROR, "deterministic", ("rubric",),
             "The judge answers each criterion Yes/No; a trailing qualifying statement is not gradable."),
    RuleInfo("R003", "Every criterion carries a point value", ERROR, "deterministic", ("rubric",),
             "Rubric gen or a contributor edit sometimes drops the points."),
    RuleInfo("R004", "Point sign matches the section", ERROR, "deterministic", ("rubric",),
             "Pitfalls are negative; Output Validation / Perturbation / Presentation / Model Integration are positive."),
    RuleInfo("R005", "Point magnitude within bounds", WARNING, "deterministic", ("rubric",),
             "|points| between 1 and 5 for scored criteria (configurable); pitfalls up to 10."),
    RuleInfo("R006", "Expected sections present and non-empty", ERROR, "deterministic", ("rubric",),
             "Output Validation, Perturbation, Presentation and Pitfalls each have at least one criterion; no declared section is empty."),
    RuleInfo("R007", "Enough criteria", WARNING, "deterministic", ("rubric",),
             "At least 10 criteria in total; Output Validation and Perturbation carry the bulk."),
    RuleInfo("R008", "No verbatim duplicate criteria", ERROR, "deterministic", ("rubric",),
             "Same text twice (after whitespace/case normalisation) double-counts points."),
    RuleInfo("R009", "Perturbation criteria are well-formed", ERROR, "deterministic", ("rubric",),
             "'If <input> on <tab> is changed from A to B, does <output> update to Z (±tol)?' with a numeric from, to and target."),
    RuleInfo("R010", "Numeric targets carry exactly one tolerance", WARNING, "deterministic", ("rubric",),
             "Every value check states a tolerance (±2% numbers, ±0.05 multiples, ±0.5pp percentages); stating it twice widens the band."),
    RuleInfo("R011", "No stacking outside Presentation", WARNING, "deterministic", ("rubric",),
             "One check per criterion; several independent targets or clauses in one question zero multiple points on one miss."),
    RuleInfo("R012", "No vague, non-self-contained wording", WARNING, "deterministic", ("rubric",),
             "'correct', 'consistent', 'appropriate', 'properly' without a tab / line-item anchor is not gradable."),
    RuleInfo("R013", "Relative tolerance on a zero target", WARNING, "deterministic", ("rubric",),
    ...
    RuleInfo("R015", "Weighting is sane", INFO, "deterministic", ("rubric",),
             "Flags extreme spread inside a section (max/min points ≥ 3x) for a human look."),
    RuleInfo("R016", "Error-value pitfall present", WARNING, "deterministic", ("rubric",),
