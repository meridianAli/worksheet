...
RULES = [
    RuleInfo("K001", "Scanner: gold has no external links, broken refs, #NAME?", ERROR, "deterministic", (),
             "Errors in the GOLD block; the same in the INPUT are reported as info only."),
    RuleInfo("K002", "Scanner: gold has no hidden sheets or images", WARNING, "deterministic", (),
...
def run(ctx, report):
    ...
    for p in scans:
        ...
        if real_ext:
            report.add(Finding("K001", ERROR if role == "gold" else INFO, f"{role}: {len(real_ext)} external link(s).", file=f, evidence=str(real_ext)[:200]))
        elif ext:
        ...
        if cnt("cellsWithBrokenReferences"):
            report.add(Finding("K001", ERROR if role == "gold" else INFO, f"{role}: {cnt('cellsWithBrokenReferences')} cell(s) with broken references.", file=f, evidence=str(d.get("cellsWithBrokenReferences"))[:200]))
        if cnt("nameErrors"):
            report.add(Finding("K001", ERROR if role == "gold" else INFO, f"{role}: {cnt('nameErrors')} #NAME? error(s).", file=f, evidence=str(d.get("nameErrors"))[:200]))
        if d.get("hiddenSheets"):
            report.add(Finding("K002", WARNING if role == "gold" else INFO, f"{role}: hidden sheets {d['hiddenSheets']}", file=f))
        if d.get("sheetsWithImages"):
            report.add(Finding("K002", WARNING if role == "gold" else INFO, f"{role}: embedded images on {d['sheetsWithImages']}", file=f))
