...
"""Run the coverage test for every task: diff the workbooks, audit the script.
  ...
  python3 tests/run_tests.py                # all five
  python3 tests/run_tests.py --task 2266fdca
  python3 tests/run_tests.py --rebuild      # re-diff the workbooks first (slow)
...
def wb(task, role):
    hits = glob.glob(os.path.join(ROOT, 'wb', f'{task}_{role}_*.xlsx'))
    if not hits:
        sys.exit(f'missing workbook for {task} ({role}) - run tests/fetch_task_workbooks.py')
    return hits[0]
...
def main():
    ...
    for t in tasks:
        ...
        cmd = [sys.executable, os.path.join(HERE, 'coverage.py'), '--diff', dj,
               '--input', wb(t, 'user'),
               '--script', os.path.join(ROOT, 'scripts', f'{t}.md')]
        if a.quiet:
            cmd.append('--quiet')
