-- Independent cross-check: August payable hours booked by strict prompt_taskers
WITH internal AS (
...
seg AS (
  ...
  WHERE r.user_id IN (SELECT user_id FROM pt)
    AND r.user_id NOT IN (SELECT id FROM internal))
SELECT ROUND((sum(secs)/3600.0)::numeric,1) AS aug_payable_hours,
       ...
       ROUND((sum(secs/3600.0 * COALESCE((
         ...
         ORDER BY (cr.user_id IS NOT NULL) DESC, cr.effective_from DESC LIMIT 1),0))/100.0)::numeric,0) AS aug_cost_usd
FROM seg WHERE day BETWEEN '2026-08-01' AND '2026-08-31'
